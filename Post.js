...
import {
    Comment, Icon, Popover, message, Tooltip, Avatar, List, Form, Button, Input
} from 'antd';
...
export default class Post extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            action: this.props.likes.length > 0
                && this.props.likes.find(id => id === this.props.currentUser.ID) ? "liked" : null,
            comments: [],
            submitting: false,
            value: '',
        }
    ...
    render() {
        ...
        var names = this.props.users.filter(user => (this.props.likes.find(id => parseInt(id) === parseInt(user.idUser))))
        names = names.map(user => (user.nom + " " + user.prenom))
        ...
        const liker = (
            <div>
                <List
                    dataSource={names}
                    itemLayout="vertical"
                    renderItem={props => <div><span>{props}</span></div>}
                >
                </List>
            </div>
        );
        ...
        const actions = [
            <span>
                <Tooltip >
                    <Popover content={liker} title="Likers">
                        <Icon
                            type="like"
